...
const normalizeFromBackend = (p = {}) => {
  ...
  return {
    ...
    thumbnail: thumbnail || "",
    price: computedPrice,
    categoryId: p.categoryId ?? null,                 // chỉ là ID
    categoryName: p.categoryName || p.category || "", // nếu BE có trả tên
    createdAt: p.createdAt,
    totalStock: totalStock ?? 0,
    rating: p.rating ?? 5,
    sales: p.sales ?? 0,
...
export const adminProductService = {
  ...
  async createProduct(productData) {
    ...
    mockProducts.unshift(newProduct);
    try { localStorage.setItem("anta_admin_products_v2", JSON.stringify(mockProducts)); } catch {}
    return { success: true, data: normalizeFromBackend(newProduct), message: "Thêm sản phẩm thành công (mock)" };
  },
