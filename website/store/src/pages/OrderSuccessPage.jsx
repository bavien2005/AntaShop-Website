...
export default function OrderSuccessPage() {
  ...
  const paymentMethod = normalizePaymentMethodKey(
    location.state?.paymentMethod ||               // ✅ ưu tiên state
  ...
  const paidAmount = (() => {
    const n = Number(location.state?.paidAmount);
    if (!Number.isNaN(n) && n > 0) return Math.round(n);

    const fallback = Number(orderData?.paidAmount || orderData?.raw?.paidAmount || 0);
    if (!Number.isNaN(fallback) && fallback > 0) return Math.round(fallback);

    return Math.round(Number(orderData?.total || 0));
  })();
                  ...
                  <div className="payment-display">
                    ...
                    <div className="payment-details">
                      ...
                      {paymentMethod === 'cod' ? (
                        ...
                        <p className="payment-confirmed">✓ Đã xác nhận thanh toán thành công với số tiền: {orderData.total.toLocaleString()} xđ
                        <tr>
                          Chúng tôi sẽ liên hệ với bạn sớm nhất để giao hàng cho bạn!</tr></p>
                      ) : (
