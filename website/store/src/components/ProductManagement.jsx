...
export default function ProductManagement({ activeSubTab, setActiveSubTab, onDataChange }) {
  ...
  const normalizeFilters = (raw) => {
    ...
    if (
      trimmed.quantityMin !== undefined &&
      trimmed.quantityMax !== undefined &&
      trimmed.quantityMin > trimmed.quantityMax
    ) {
  ...
  const getProductImage = (product) =>
    product?.thumbnail || product?.image || (product?.images && product.images[0]) || placeholderImage;
    ...
    filteredProducts.forEach((item) => {
      ...
      if (Array.isArray(item.variants) && (item.name || item.id)) {
        ...
        if (!map.has(pid)) map.set(pid, { product: item, variants: Array.isArray(item.variants) ? item.variants : [] });
          ...
          const e = map.get(pid);
          e.product = { ...e.product, ...item };
          e.variants = e.variants.concat(item.variants || []);
        }
        return;
      }
      ...
      if (productId !== null && productId !== undefined) {
        ...
        if (!map.has(pid)) {
          const productObj = {
            ...
            category: item.category ?? item.categoryName ?? item.cat ?? "",
  ...
  const computeTotalStock = (product, variants) => {
    ...
    if (Array.isArray(variants) && variants.length)
      return variants.reduce((s, v) => s + (Number(v.stock ?? v.quantity ?? 0) || 0), 0);
    return 0;
  };
    ...
    <div className="product-management">
      <div className="product-management-content">
        ...
        <div className="tabs-section">
          ...
          <button
            className={`tab-button ${activeSubTab === "add-product" ? "active" : ""}`}
            onClick={() => {
              setEditingProduct(null);
              setActiveSubTab("add-product");
            }}
          ...
          <button className="tab-button" onClick={() => setActiveSubTab("violations")}>
            Vi phạm
          </button>
        </div>
                ...
                return (
                  <React.Fragment key={pid}>
                    <tr className="table-row parent-row" role="row">
                      ...
                      <td className="category-cell col-category">{product.category || product.categoryName || "—"}</td>

                      <td className="price-cell col-price">{priceText}</td>

                      <td className="quantity-cell col-quantity">
                        <span className={totalStock <= LOW_STOCK_THRESHOLD ? "low-stock-badge" : ""}>{totalStock}</span>
                      </td>

                      <td
                        className="variants-cell col-variants"
                        title={variantsCount ? `${variantsCount} variants` : "Không có variant"}
                      >
                        {variantsCount}
                      </td>
                      ...
                      <tr className="variant-subrow">
                        <td colSpan={8} className="variant-subcell">
                          <div className="variant-panel">
                            <div className="variant-panel-title">Variants ({variantsCount})</div>

                            <div className="variant-grid">
                              {variants.map((v, idx) => {
                                const vid = v.id ?? `${pid}-v-${idx}`;
                                const vPriceText =
                                  v.price !== undefined && v.price !== null ? `${formatPrice(Number(v.price))} VNĐ` : "—";
